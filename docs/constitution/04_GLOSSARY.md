...
| **hosted demo** | Controlled, non-durable Railway try-before-install endpoint at `tube-bridge-production.up.railway.app`. Enforces 5 attempted Data API operations per Railway-overwritten IP/process and a 10-minute corpus deadline; no volume, accounts, backups, SLA, or managed-hosting promise. |
...
| **publication gate** | Per-surface P0/P1 checks in `docs/planning/PUBLICATION_READINESS.md`. Self-hosted core and disposable-demo P0 gates passed independently; conditional P1 items remain triaged. |
